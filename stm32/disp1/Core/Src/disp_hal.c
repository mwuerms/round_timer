...
void disp_hal_send_data_buffer(uint8_t *data_buff, uint32_t buff_size)
{
	...
	*/
	/*while(buff_size) {
		while((DISP_SPI->SR & SPI_FLAG_TXE) == 0); // wait until done
		if(buff_size > 1) {
			// load 2 x 8 bits into DR
			*((__IO uint16_t *)&DISP_SPI->DR) = *((uint16_t *)data_buff); // send
			data_buff += 2;
			buff_size -= 2;
		}
		else {
			// load only uint8_t into DR
			*((__IO uint8_t *)&DISP_SPI->DR) = *((uint8_t *)data_buff); // send
			data_buff += 1;
			buff_size -= 1;
		}
	}
...
/*
// using HAL, works but very very slowly, lots of pauses
...
void disp_hal_send_cmd(uint8_t cmd)
{
	...
	uint32_t spi_transmit(SPI2, &cmd, 1);
